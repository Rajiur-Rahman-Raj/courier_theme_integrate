<script>
		...
		$('.OCParcelTypeWiseShippingRate').each(function (key, value){
			...
			$.ajax({
				...
				success: function (response) {
					...
					parseFloat($('.calculateShippingCost').text(`${basic.currency_symbol}${totalShippingCost}`)).toFixed(2);
					parseFloat($('.calculateTax').text(`${basic.currency_symbol}${totalTax}`)).toFixed(2);
					parseFloat($('.calculateInsurance').text(`${basic.currency_symbol}${totalInsurance}`)).toFixed(2);


					parseFloat($('.calculateShippingCostInput').val(totalShippingCost)).toFixed(2);
					parseFloat($('.calculateTaxInput').val(totalTax)).toFixed(2);
					parseFloat($('.calculateInsuranceInput').val(totalInsurance)).toFixed(2);
		...
		if ($('input[name="shipment_type"]:checked').val() === "condition") {
			...
			$.ajax({
				...
				success: function (response) {
					...
					@endphp
					let basic = @json($basic);
					$('.calculateShippingCostInput').val(response.cash_on_delivery_cost);
					$('.calculateShippingCost').text(`${basic.currency_symbol}${response.cash_on_delivery_cost}`);
	...
	function finalTotalAmountCalculation(conditionShippingCost = null){
		...
		if(conditionShippingCost != null){
			shippingCost = conditionShippingCost;
		}else{
			shippingCost = parseFloat($('.calculateShippingCostInput').val());
		}

		let calculateTax = parseFloat($('.calculateTaxInput').val());
		let calculateInsurance = parseFloat($('.calculateInsuranceInput').val());
		let calculatePickupCost = parseFloat($('.calculatePickupCostInput').val());
		let calculateSupplyCost = parseFloat($('.calculateSupplyCostInput').val());

		let totalCost = subTotal + shippingCost + calculatePickupCost + calculateSupplyCost + calculateTax + calculateInsurance;
		$('.totalCost').text(`${basic.currency_symbol}${totalCost}`);
	}
