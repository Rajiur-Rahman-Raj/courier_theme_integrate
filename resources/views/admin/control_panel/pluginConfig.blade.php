														...
														<div class="card-body">
															<div class="row align-items-center">
																<div class="col-md-2"><img
																		...
																		class="w-25"></div>
																<div class="col-md-6">@lang('Manual reCAPTCHA protects your website from fraud and abuse.')</div>
																<div class="col-md-4">
																	<div class="d-flex flex-wrap align-items-start ">
												...
												<div class="col-lg-10">
													<div class="card mb-4 shadow">
														<div class="card-body">
															<div class="row justify-content-between align-items-center">
																<div class="col-md-3"><img
																		src="{{ asset('assets/upload/analytics.png') }}"
																		class="w-25"></div>
																<div
																	class="col-md-6">@lang('Google Analytics is a web analytics service offered by Google.')</div>
																<div class="col-md-3"><a
																		href="{{ route('google.analytics.control') }}"
																		class="btn btn-sm btn-primary"
																		target="_blank">@lang('Configuration')</a></div>
															</div>
														</div>
...
@push("extra_scripts")
	<script>
		...
		$(document).ready(function() {
			...
			$('#selectgroup-input-off').on('click',function() {
				$.ajax({
					...
					success: function(response) {
						Notiflix.Notify.success("{{trans('Recaptcha has been disabled')}}");
					},
			...
			$('#selectgroup-input-on').on('click',function() {
				$.ajax({
					url: "{{ route('active.recaptcha') }}",
					type: "GET",
					data: {
					...
					success: function(response) {
						Notiflix.Notify.success("{{trans('Recaptcha has been enabled')}}");
					},
					error: function(error) {
					}
				});
			...
			$('#selectgroup-manual-off').on('click',function() {
				$.ajax({
					...
					success: function(response) {
						Notiflix.Notify.success("{{trans('Manual captcha has been disabled')}}");
					},
					error: function(error) {
					}
				});
			...
			$('#selectgroup-manual-on').on('click',function() {
				$.ajax({
					url: "{{ route('active.manual.recaptch') }}",
					type: "GET",
					data: {
					...
					success: function(response) {
						Notiflix.Notify.success("{{trans('Manual captcha has been enabled')}}");
					},
					error: function(error) {
					}
				});
