										...
										@foreach($notifyTemplates as $template)
											<tr>
												...
												<td data-label="@lang('Status')">
													<span class="badge badge-pill badge-{{ ($template->firebase_notify_status == 1) ?'success' : 'danger' }}">
														{{ ($template->firebase_notify_status == 1) ? trans('Active') : trans('Deactivate') }}
													</span>
												</td>
												<td data-label="@lang('Action')">
													<a href="{{ route('push.notify.template.edit',$template->id) }}" class="btn btn-sm rounded-circle btn-outline-primary"><i class="fas fa-edit"></i> </a>
												</td>
											</tr>
										@endforeach
										</tbody>
