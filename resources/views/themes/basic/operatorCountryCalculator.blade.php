				...
				<div class="row gy-4 justify-content-center">
					<div class="col-md-4 col-sm-6">
						<div class="progress_box text-center">
							<div class="number bg_highlight active number1  {{ lastUriSegment() == 'operator-country' ? 'bg_highlight active' : '' }}  mx-auto"><a
									href="{{ route('shippingCalculator.operatorCountry') }}"><i class="fas fa-truck"></i> {{ optional(basicControl()->operatorCountry)->name }}</a>
							</div>
						</div>
					</div>
					...
					<div class="col-md-4 col-sm-6">
						<div class="progress_box text-center">
							<div class="number {{ lastUriSegment() == 'internationally' ? 'bg_highlight active' : '' }}  number2 mx-auto"><a
									href="{{ route('shippingCalculator.internationally') }}"><i class="fas fa-plane-departure"></i> @lang('Internationally')</a>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="col-12">
					<form action="">
						<div
							...
							<div class="form-check">
								<input class="form-check-input" type="radio" id="shipmentTypeCondition"
									   name="shipment_type"
									   value="condition" @if(old('shipment_type') === 'condition') checked @endif>
								<label class="form-check-label" for="shipmentTypeCondition">
									@lang($shipmentTypeList[2]['shipment_type'])
									<small>(@lang($shipmentTypeList[2]['title']))</small>
								</label>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-sm-6 mt-20">
								<label for="from_city_id">@lang('Select City') <span
										class="text-muted font-weight-bold"><sub>(@lang('optional'))</sub></span></label>
								<select name="from_city_id"
										class="form-select @error('from_city_id') is-invalid @enderror selectedFromCity"
							...
							<div class="col-sm-6 mt-20">
								...
								<select name="from_area_id"
										class="form-select @error('from_area_id') is-invalid @enderror selectedFromArea"
							...
							<div class="col-sm-6 mt-20">
								<label for="to_area_id">@lang('Select Area') <span
										class="text-muted font-weight-bold"><sub>(@lang('optional'))</sub></span>
								</label>
								<select name="to_area_id"
										class="form-select @error('to_area_id') is-invalid @enderror selectedToArea"
							...
							<div class="row gy-4">
								<div class="col-lg-3 col-sm-6 mx-auto shippingCostBox">
									<div class="costing_box text-center">
										<input type="hidden" name="sub_total" value="{{ old('sub_total') ?? '0' }}"
											   class="form-control bg-white text-dark subTotal" data-subtotal="{{ old('sub_total') }}"
											   readonly>

										<p>@lang('SHIPPING COST')</p>
										<input type="hidden" class="calculateShippingCostInput" name="calculate_shipping_cost_input" value="{{ old('calculate_shipping_cost_input') ?? '0' }}">
								...
								<div class="col-lg-3 col-sm-6 mx-auto d-none pickupCostBox">
									<div class="costing_box text-center">
										...
										<input type="hidden" class="calculatePickupCostInput" name="calculate_pickup_cost_input" value="{{ old('calculate_pickup_cost_input') ?? '0' }}">
								...
								<div class="col-lg-3 col-sm-6 mx-auto d-none supplyCostBox">
									<div class="costing_box text-center">
										...
										<input type="hidden" class="calculateSupplyCostInput" name="calculate_supply_cost_input" value="{{ old('calculate_supply_cost_input') ?? '0' }}">
								...
								<div class="col-lg-3 col-sm-6 mx-auto taxCostBox">
									<div class="costing_box text-center">
										<p>@lang('TAX COST')</p>
										<input type="hidden" class="calculateTaxInput" name="calculate_tax_cost_input" value="{{ old('calculate_tax_cost_input') ?? '0' }}">
								...
								<div class="col-lg-3 col-sm-6 mx-auto insuranceCostBox">
									<div class="costing_box text-center">
										...
										<input type="hidden" class="calculateInsuranceInput" name="calculate_insurance_cost_input" value="{{ old('calculate_insurance_cost_input') ?? '0' }}">
			...
			<div class="row">
				<div class="col-12">
					<div class="btn_area d-flex justify-content-end">
						<a href="{{route('user.createShipment', ['shipment_type' => 'operator-country', 'shipment_status' => 'all'])}}" class="cmn_btn">@lang('Create Shipment')</a>
					</div>
				</div>
			</div>
		</div>
	...
	<script>
		...
		function formHandlingByShipmentType() {
			if ($('input[name="shipment_type"]:checked').val() === "drop_off") {
				$('.ParcelInfoArea').removeClass('d-none');
				...
				$('.ParcelInfoArea').addClass('d-none');
