													...
													@forelse($allCities as $key => $city)
														<tr>
															<td data-label="@lang('Country')">
																@lang(optional($city->country)->name)
															</td>

															<td data-label="@lang('State')">
																@lang(optional($city->state)->name)
															</td>
															...
															<td data-label="@lang('City')">
																<a href="{{ route('cityList', ['show-city-list', optional($city->state)->id]) }}"
																   class="text-decoration-underline">
																	<span class="badge badge-light">{{ optional($city->state)->getTotalCity() }}</span>
																</a>
															</td>
													...
													@empty
														<tr>
															<td colspan="100%" class="text-center">
																...
																<p class="text-center no-data-found-text">@lang('No City Found')</p>
															</td>
														</tr>
													@endforelse
													</tbody>
			...
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title text-dark font-weight-bold"
						id="primary-header-modalLabel">@lang('Add City')</h4>
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				</div>
				<form action="{{ route('cityStore') }}" method="post" enctype="multipart/form-data">
					@csrf
...
@section('scripts')
	...
	<script>
		...
		$(document).ready(function () {
			...
			function getSeletedCountryState(value) {
				$.ajaxSetup({
					headers: {
