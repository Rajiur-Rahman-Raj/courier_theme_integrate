		...
		if ($request->hasAny(array_keys(config('templates.template_media')))) {
			...
			foreach (config('templates.template_media') as $key => $media) {
				...
				if ($request->hasFile($key)) {
					$templateSection = config('templates')[$section];
					$size = array_key_exists('size', $templateSection) ? config('templates')[$section]['size'][$key] : null;
					$image = $this->fileUpload($purifiedData[$key][$language], config('location.template.path'), $templateMedia->driver, null, $old_data, $size);
					if ($image) {
						$templateMediaDescription[$key] = $image['path'];
